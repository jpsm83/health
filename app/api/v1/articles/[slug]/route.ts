...
export const GET = async (
  ...
  try {
    ...
    if (!locale) {
      return new NextResponse(
        JSON.stringify({ message: "Locale parameter is required" }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    // Find article by slug in the specific locale
  ...
  try {
    ...
    if (fileEntries.length > 0) {
      ...
      if (newImages.length > 0) {
        const folder = `/${category}/${article._id}`;
